    ...
    if @find_guest.present? && @find_guest.first.presence != true
      ...
      respond_to do |format|
        format.json { render json:{
          ...
          nama: "#{@find_guest[0].custom_two_text} - #{@find_guest[0].nama}",
...
def new
  ...
  def dashboard_vip
    content = {}
    @total_guest_vip = Guest.joins(:event).where("events.status=true").where("guests.status = 'VIP' OR guests.status = 'VVIP' ").where("guests.presence = TRUE").count
    # @guest_vip = Guest.joins(:event).where("events.status=true AND kategori = 'VIP'")
    @guest_vip = Guest.joins(:event)
    .where("events.status=true AND presence=true ").
    where("guests.status = 'VIP' OR guests.status = 'VVIP' ")
              .select("
                        nama as Nama,
                        kategori,
                        guests.status,
